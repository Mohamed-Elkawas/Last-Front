...
function PlaygroundPaymentSubmittedContent() {
  ...
  const playgroundLocation =
    booking?.playground?.location[language] || t("common.notProvided")
  const dateLabel =
    booking?.playground?.dateLabel || t("common.notProvided")
  ...
  const paymentNumber =
    booking?.payment?.paymentNumber || t("common.notProvided")
  const transactionReference =
    booking?.payment?.transactionReference || t("common.notProvided")
        ...
        <Card className="w-full max-w-md text-center">
          <CardContent className="p-8">
            ...
            <h1 className="mt-6 text-2xl font-bold text-foreground">
              {t("successPlayground.title")}
            </h1>

            <p className="mt-2 text-3xl font-bold text-primary">
              {t("successPlayground.amountLine", { total })}
            </p>

            <p className="mt-4 text-muted-foreground">
              {t("successPlayground.subtitle")}
            </p>
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="font-medium text-amber-800">
                    {t("successPlayground.statusTitle")}
                  </p>
                  <p className="text-sm text-amber-700">
                    {t("successPlayground.statusBody")}
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="mt-6 rounded-lg bg-muted p-4 text-start">
              <h3 className="font-medium text-foreground">
                {t("successPlayground.detailsTitle")}
              </h3>
              ...
              <div className="mt-3 space-y-2">
                ...
                <div className="flex flex-wrap items-center gap-2 text-sm">
                  ...
                  <span className="font-medium">{slotsLabel}</span>
                </div>
            ...
            <div className="mt-6 flex flex-col gap-3">
              <Button asChild>
                <Link href="/bookings">
                  {t("successPlayground.viewBookings")}
                </Link>
              </Button>

              <Button variant="outline" asChild>
                <Link href="/playgrounds">
                  {t("successPlayground.backPlaygrounds")}
                </Link>
              </Button>
            </div>
          </CardContent>

...
function buildSession(accountType: "player" | "owner", userId?: string): AuthSession {
  const roles: AuthSession["roles"] = accountType === "owner" ? ["owner"] : ["player"]
  return {
...
const request = {
  ...
  const session = buildSession(payload.accountType, userId)
...
export async function registerOwner(
  ...
  const response = await postToApi<OwnerRegisterApiRequest, AuthApiResponse<string>>(
